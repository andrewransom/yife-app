...
import type { EntityBacklink } from './types';

export function useEntityBacklinksQuery(
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await client.rpc('get_entity_backlinks', {
        p_entity_id: id,
        p_role_view:
